<!-- sidebar -->
<aside class="sidebar" role="complementary">

	<?php if ( is_active_sidebar('widgetarea1') ) : ?>
		<?php dynamic_sidebar( 'widgetarea1' ); ?>
	<?php else : ?>
 		
 		<div class="widget widget-popular-hotels">
 			<h6>Популярные гостиницы Киева</h6>
 			<ul>
 				<li class="hotel-has-halfprice hotel-has-online">
 					<a href="#">
 						<img src="" alt="" class="feature-photo">
 						<span class="hotel-rating"></span>
 						<h5>Гостиница Ramada Encore Kiev</h5>
 						<p class="price">от <span class="num">90</span> грн</p>
 						<p class="timers">за ночь</p>
 					</a>
 				</li>
 				<li class="hotel-has-online">
 					<a href="#">
 						<img src="" alt="" class="feature-photo">
 						<span class="hotel-rating"><i class="fa fa-star"></i><i class="fa fa-star"></i><i class="fa fa-star"></i><i class="fa fa-star"></i><i class="fa fa-star"></i></span>
 						<h5>Гостиница Ramada Encore Kiev</h5>
 						<p class="price"><span class="price-old">от 200 грн</span><span class="price-new">от 100 грн</span></p>
 						<p class="timers">за ночь</p>
 					</a>
 				</li>
 				<li class="">
 					<a href="#">
 						<img src="" alt="" class="feature-photo">
 						<span class="hotel-rating"></span>
 						<h5>Гостиница Ramada Encore Kiev</h5>
 						<p class="price">от <span class="num">90</span> грн</p>
 						<p class="timers">за ночь</p>
 					</a>
 				</li>
 				<li class="hotel-has-halfprice">
 					<a href="#">
 						<img src="" alt="" class="feature-photo">
 						<span class="hotel-rating"></span>
 						<h5>Гостиница Ramada Encore Kiev</h5>
 						<p class="price">от <span class="num">90</span> грн</p>
 						<p class="timers">за ночь</p>
 					</a>
 				</li>
 				<li class="">
 					<a href="#">
 						<img src="" alt="" class="feature-photo">
 						<span class="hotel-rating"><i class="fa fa-star"></i><i class="fa fa-star"></i><i class="fa fa-star"></i></span>
 						<h5>Гостиница Ramada Encore Kiev</h5>
 						<p class="price">от <span class="num">90</span> грн</p>
 						<p class="timers">за ночь</p>
 					</a>
 				</li>
 			</ul>
 			<a href="#" class="veiw-all">Смотреть все отели »</a>
 		</div>
 		<!-- /.widget widget-popular-hotels -->
		
 
	<?php endif; ?>
</aside>
<!-- /sidebar -->
